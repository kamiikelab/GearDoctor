...
実装は PC（このリポジトリがある WSL）で行います。**Android は GitHub Releases の APK で配り、Google Play には出しません。** iPhone は App Store（Codemagic → App Store Connect）です。
...
連携済みなら、ホームの「走行を追加」から「前回から1年取り込む」で走行と自転車を取ります。Strava開始日の変更は、取り込んだ走行の消去を伴います。手入力は残します。取り直しは「前回から1年取り込む」です。失敗時の続きは [Strava 連携](Strava連携.md) の未確定事項です。

## 開発の流れ（全体）

| 段階 | どこで動かすか | 目的 |
| --- | --- | --- |
| テスト | 手元の `flutter test` と GitHub Actions | ロジックの確認。PUSH のたびに Actions でも回す |
| PC | `flutter run -d linux` | 画面操作の確認 |
| 実機（Android） | GitHub Releases の APK | スマホと同じ形 |
| 実機（iPhone） | TestFlight / App Store | Codemagic の IPA |
| 配布（Android） | [GitHub Releases](https://github.com/kamiikelab/GearDoctor/releases) の APK | Google Play には出さない |
| 配布（iPhone） | App Store Connect（今までの流れ） | GitHub の Release は使わない |

ソースは一つです。**GitHub の Release は Android の APK だけ**です。タグを切っても App Store の審査は始まりません。iPhone は今までどおり Codemagic → App Store Connect です。Codemagic の設定は触らず、Release やタグで iPhone ビルドを自動起動しないでください。
...
渡すのはソースではなく **Android の APK** です。案内するのは上記の「2. APK ファイルを渡して入れる」です。USB デバッグは不要です。新しい Android 版を配るときは、APK を作って同じ Releases に添付します。iPhone は Releases に載せません。
