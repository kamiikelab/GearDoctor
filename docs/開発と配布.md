...
### 2. APK ファイルを渡して入れる（自分の実機と仲間向け）

Play ストアを経由しません。Git に置いた APK をダウンロードするか、ファイルを直接渡します。

1. APK をスマホに置く（Git の Release、USB ファイルコピー、Drive など）
...
渡すのはソースではなく APK です。USB デバッグは不要です。上記の「2. APK ファイルを渡して入れる」だけを案内します。
