...
Play ストアは使いません。Android の Chrome で、次の順に進めてください。
...
初回起動時に、画面確認用のデモ部品・走行・ギアを端末内 SQLite へ入れます。走行は手入力でも入れられます。最初の実走行でデモ走行は消えます。Strava 連携は任意です。手順は [`docs/Strava連携.md`](docs/Strava連携.md) です。`strava_secrets.json` は Git に入れません。
...
`main` への PUSH と pull request では、GitHub Actions（Linux）が同じ `flutter test` を回します。iPhone の IPA は Codemagic、Android の APK は GitHub Releases です。
